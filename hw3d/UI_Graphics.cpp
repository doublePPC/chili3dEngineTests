...
{
#include "UI_FramebufferPS.shh"
#include "UI_FramebufferVS.shh"
}

#pragma comment( lib,"d3d11.lib" )

#define CHILI_GFX_EXCEPTION( hr,note ) UI_Graphics::Exception( hr,note,_CRT_WIDE(__FILE__),__LINE__ )
...
void UI_Graphics::DrawRect(int width, int height, int xCoord, int yCoord, Color c)
{
	for (int i = 0; i < height; i++)
	{
		for (int j = 0; j < width; j++)
		{
			PutPixel(xCoord + j, yCoord + i, c);
		}
	}
}

void UI_Graphics::EndFrame()
{
	HRESULT hr;
...
std::wstring UI_Graphics::Exception::GetFullMessage() const
{
...
std::wstring UI_Graphics::Exception::GetErrorName() const
{
	// !! Usage forcé de la fonction "Wide" versus impossibilité de convertir narrow en wide
	return DXGetErrorStringW( hr );
}

std::wstring UI_Graphics::Exception::GetErrorDescription() const
{
	// !! Usage forcé de la fonction "Wide" versus impossibilité de convertir narrow en wide
	std::array<wchar_t, 512> wideDescription;
	DXGetErrorDescriptionW( hr,wideDescription.data(), wideDescription.size() );
	return wideDescription.data();
}

std::wstring UI_Graphics::Exception::GetExceptionType() const
{
